...
SurePetcare.prototype.pollStatus = function () {
  var self = this;
  var interval = self.config.poll_interval
    ? self.config.poll_interval * 1000
    : 30000;
  setTimeout(function () {
    self.PetcareApi.getStatuses(function (data) {
  ...
  switch (device.product_id) {
    case 6: // cat flap iDSCF
      ...
      if (accessory === undefined) {
        ...
        this.accessories[uuid] = new SurePetcarePetFlap(
          this.log,
          accessory instanceof SurePetcarePetFlap
            ? accessory.accessory
            : accessory,
          device,
          this.PetcareApi,
...
SurePetcare.prototype.addPets = function (pet) {
  ...
  var occupancy_flipped = this.config.occupancy_flipped
    ? this.config.occupancy_flipped
    : false;

  if (accessory === undefined) {
    this.registerOccupancySensor(pet, occupancy_flipped);
  } else {
    this.accessories[uuid] = new SurePetcareOccupancySensor(
      this.log,
      accessory instanceof SurePetcareOccupancySensor
        ? accessory.accessory
        : accessory,
      pet,
      this.PetcareApi,
...
SurePetcare.prototype.registerOccupancySensor = function (
  pet,
  occupancy_flipped
) {
  var uuid = UUIDGen.generate("PET-" + pet.id);
